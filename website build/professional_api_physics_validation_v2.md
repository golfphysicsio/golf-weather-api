...
**Not testing:** Exact yardage matches to unreported field data  
**Testing:** That our model exhibits physically correct qualitative behavior:

✅ **Structure:** Relative airspeed used for both drag and lift  
✅ **Tailwinds:** Increase carry up to an optimal point  
✅ **Headwinds:** Reduce carry proportionally  
✅ **Temperature:** Affects distance through density and compression  
✅ **Altitude:** Reduces air density, increases distance  
✅ **Combined effects:** Compound appropriately  

### What "Validation" Means Here:

- ✅ These scenarios test that our **implementation logic is correct**
- ✅ Expected ranges are **model outputs** we use for consistency checking
- ✅ Behavior is **qualitatively consistent** with published golf physics
- ❌ Not claiming these exact numbers are field-validated (v2.0 goal)
...
**To validate our physics implementation:**

1. **Check relative airspeed structure:**
   - Verify vrel is used in dynamic pressure for BOTH drag and lift
   - Confirm it's not just drag alone

2. **Verify qualitative behavior:**
   - Moderate tailwinds increase carry ✓
   - Extreme tailwinds can reduce carry (if modeled beyond 40mph) ✓
   - Headwinds reduce carry ✓
   - Temperature/altitude effects directionally correct ✓

3. **Confirm validation caps:**
   - Winds >40mph return 422 validation error ✓
   - Temps outside 32-105°F rejected ✓
   - Altitudes >8000ft rejected ✓

4. **Accept model limitations:**
   - Launch-speed approximations (not full time-step)
   - Representative coefficients (not speed-varying)
   - Appropriate for intended use case (club selection, training)

5. **Don't expect:**
   - Sub-yard precision
   - Field-validated exact distances (v2.0 goal)
   - Wind tunnel coefficient curves (v2.0 goal)

---

## 📤 Expected API Response Format

For each test scenario, the Professional API returns:
